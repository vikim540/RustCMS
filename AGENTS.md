...
> **強制約束文件**。所有代碼生成、修改、審查必須遵守。當前版本：**v1.9.5**（2026-07-22）
...
| 版本 | 日期 | 摘要 |
|------|------|------|
| v1.9.5 | 2026-07-22 | 權限清理 + 媒體庫刪除狀態 + 編輯保存比對 + 編輯器排版。徹底清除 M205 菜單 + 角色權限（M204 為現行啟用版本）、媒體庫刪除圖片時顯示 🔄 [文件名] 刪除中... 覆蓋層、文章編輯保存時自動比對修改字段（提示「此次修改了 N 處」，無修改不觸發後端）、編輯器有序列表懸掛縮進 CSS、新增 HTML 源碼按鈕（工具列 <> 圖標，textarea 模式直接編輯 HTML） |
