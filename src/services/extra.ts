...
/** 內部查詢全部自定義標籤 (ORDER BY id ASC) */
async function listAllLabels(db: D1Database): Promise<unknown[]> {
  const result = await db.prepare('SELECT * FROM ay_label ORDER BY id ASC').all();
  return result.results;
}

/** 後台自定義標籤列表 (無分頁) */
export async function handleListLabels(db: D1Database): Promise<Response> {
  return okData(await listAllLabels(db), '成功');
}

/** 公開自定義標籤列表 */
export async function handleListLabelsPublic(db: D1Database): Promise<Response> {
  return okData(await listAllLabels(db), '成功');
}
...
export async function handleCreateLabel(
  db: D1Database,
  body: { name?: string; value?: string; type?: string; description?: string },
): Promise<Response> {
  const name = body.name;
  if (!name) return err('缺少 name 參數', 1001);
  ...
  const now = nowStr();
  const result = await db.prepare(
    'INSERT INTO ay_label (name, value, type, description, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?)',
  ).bind(name, body.value || '', body.type || '1', body.description || '', now, now).run();

  if (result.meta.changes > 0) {
    return ok('自定義標籤創建成功');
  }
  return err('自定義標籤創建失敗', 1005);
}

/** 修改自定義標籤 (字段: name, value) */
export async function handleUpdateLabel(
  ...
  const sql = `UPDATE ay_label SET ${sets.join(', ')} WHERE id = ?`;
  await db.prepare(sql).bind(...binds).run();

  return ok('自定義標籤更新成功');
}

/** 批量更新自定義標籤值 (body: { labels: [{id, name, value}] }) */
export async function handleBatchUpdateLabels(
  db: D1Database,
  body: { labels?: Array<{ id?: number; name?: string; value?: string }> },
): Promise<Response> {
  const labels = body.labels;
  if (!Array.isArray(labels) || labels.length === 0) {
    return err('缺少 labels 參數', 1001);
  }
  ...
  for (const item of labels) {
    if (item.id !== undefined && item.value !== undefined) {
      await db.prepare(
        'UPDATE ay_label SET value = ?, update_time = ? WHERE id = ?',
      ).bind(item.value, now, item.id).run();
    }
  }

  return ok('自定義標籤批量更新成功');
}

/** 刪除自定義標籤 */
export async function handleDeleteLabel(db: D1Database, id: number): Promise<Response> {
  await db.prepare('DELETE FROM ay_label WHERE id = ?').bind(id).run();
  return ok('自定義標籤刪除成功');
}

// ============================================================================
// 模塊 6: 留言管理 (ay_message)
// ============================================================================
